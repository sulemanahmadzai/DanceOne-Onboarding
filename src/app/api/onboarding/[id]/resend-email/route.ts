...
export async function POST(
  ...
  try {
    ...
    // Check if request is waiting for candidate
    if (onboardingRequest.status !== OnboardingStatus.WAITING_FOR_CANDIDATE) {
      return NextResponse.json(
        { error: "Request is not waiting for candidate" },
        { status: 400 }
      );
    }
    ...
    if (!token) {
      return NextResponse.json(
        { error: "No token found for this request" },
        { status: 404 }
      );
    }

    // Send email to candidate
    const candidateLink = `${process.env.NEXT_PUBLIC_APP_URL}/candidate/${token.token}`;
